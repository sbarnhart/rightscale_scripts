...
## Terminate the script if no tags were found
...
# @client.tags.by_tag(:resource_type => 'instances', :tags => [tags]) will return a resource_tag object
# This resource_tag object contains all the instances/resources that contain that tag.
## If only one resource is found, the `.show.resource` method will return a single object and I manually make an array out of it using [ ] (line 47)
## If more than one resource is found, an array is returned
...
# `instances` must be an array from this point forward (should have been handled alreadu)


# Setup `tasks` Array to hold task objects generated by run_executable
tasks=[]
# Loop through the instances array and trigger the script to run on each
...
while tasks.count > 0 do
	tasks.each do |task|
		# Once the task summary contains "completed :", remove the object from the tasks array.
